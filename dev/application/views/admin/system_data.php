                ...
                <table id="example1" class="table table-bordered table-striped">
                 ...
                 <?php $cnt=''; foreach($system_data_list as $row): $cnt++; ?>
                  <tr ><!-- data-href="add-party.php" class="cursor-pointer" -->
                   ...
                   <td>
                    <?php $originalDate = $row['follow_up_date'];
                  ...
                  <td><?php echo $row['contact_to']; ?></td>
                   ...
                   <td>
                    <?php 
                      $data = $this->db->query("select created_at as t from system_data where id='".$row['id']."'")->result();
                      $time ='';
                      ...
                      $end = new DateTime($from_time);

                      $interval = $start->diff($end);
                     
                     //$hrs = $interval->days * 24 + $interval->h;

                      //echo $hrs." hours ".$interval->format('%i')." minutes"; die;

                      $total_time = $interval->format('%i');
                      $hrs = $interval->format('%h');
                      //echo $interval->format('%h hours %i minutes %S seconds'); die;
                      /*echo $interval->format('%i'); die;
                      $total_time = $interval->format('%i');
                      print_r($total_time); die;*/
                      //print_r($interval->format('%h')); die;
                     // print_r($row['status']); die;
                      if($hrs>0 || $total_time>=30){

                        if($row['status']==''){
                          ?>
                          <a href="javascript:;" id="colourButton" class="btn btn-danger">
                            <em class="fa fa-bell red-notification"></em>
                          </a>
                      <?php }else{
